      ...
      case 'plyr':
        ...
        loadScript('https://cdn.plyr.io/3.7.8/plyr.polyfilled.js').then(function () {
          try {
            ...
            var seekStep = parseInt(config.seek_step_seconds, 10);
            if (!isFinite(seekStep) || seekStep <= 0) seekStep = 10;
            var defaultSpeed = Number(config.playback_speed_default);
            if (!isFinite(defaultSpeed) || defaultSpeed <= 0) defaultSpeed = 1;
            ...
            var controls = config.plyr_hideControls ? [] : ['play-large', 'play', 'rewind', 'fast-forward', 'progress', 'current-time', 'duration', 'mute', 'volume', 'settings', 'fullscreen'];
            var plyrOptions = {
              controls: controls,
            ...
            var mountQuality = hostEl && hostEl.querySelector ? hostEl.querySelector('[data-role="quality"]') : null;
            var mountPlayback = hostEl && hostEl.querySelector ? hostEl.querySelector('[data-role="playback"]') : null;
            if (mountQuality) mountQuality.style.display = 'none';
            if (mountPlayback) mountPlayback.style.display = 'none';

            var startPlyr = function () {
              var plyrInstance = new window.Plyr(videoEl, plyrOptions);
              plyrInstance.on('timeupdate', reportTime);
            };

            if (isM3u8Url(videoEl.src) && config.hls_quality_enabled !== false) {
              var hlsCdn = String(config.hls_js_cdn || 'https://cdn.jsdelivr.net/npm/hls.js@1.5.15/dist/hls.min.js');
              loadScript(hlsCdn).then(function () {
                try {
                  var HlsCtor = window.Hls;
                  if (!HlsCtor || !HlsCtor.isSupported || !HlsCtor.isSupported()) return startPlyr();
                  var hls = new HlsCtor({
                    startLevel: (config.hls_start_level != null ? config.hls_start_level : -1),
                    capLevelToPlayerSize: config.hls_cap_level_to_player_size !== false
                  });
                  videoEl.__daop_hls = hls;
                  hls.loadSource(String(videoEl.src));
                  hls.attachMedia(videoEl);
