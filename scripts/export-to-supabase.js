...
async function main() {
  ...
  console.log('Export to Supabase — scope:', scope, '| batch upsert:', batchSize, '| concurrency:', concurrency);
  ...
  for (let i = 0; i < movies.length; i += batchSize) {
    const chunk = movies.slice(i, i + batchSize).map(movieToRow);
    ...
    console.log('   Upserted movies:', upserted, '/', movies.length);
  }
  ...
  console.log('Done. Movies upserted:', upserted, '| episode rows inserted:', epInserted);
}
