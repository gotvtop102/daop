# Build App GoTV – Build Guide (Android APK + iOS TestFlight)

Tài liệu này hướng dẫn **cài đặt môi trường** và **build app** từ thư mục **Build App GoTV**.

Thư mục này dùng **Capacitor** để bọc website thành app (URL-only):
- App load **remote URL** (website) qua `--url` hoặc biến `CAPACITOR_SERVER_URL`
- Android build ra **2 bản**: `phone` và `tv` (2 package riêng)
- iOS (tuỳ chọn) build và upload **TestFlight** (bắt buộc macOS + Xcode)

> Ghi chú: **Admin / Website tĩnh** là dự án khác. Hướng dẫn này chỉ tập trung vào **build app từ URL**.
...
- Cài dependencies trong thư mục `app/` (bạn có thể bỏ qua nếu đã có `app/node_modules/`).
...
APK được copy ra:
- `app/dist/apk/` (tính từ thư mục Build App GoTV)
