...
1. **update-data.yml** – Chạy hàng ngày (cron), gọi `npm run build` với secrets (TMDB, OPhim, Supabase Admin, R2). Commit và push thay đổi vào repo. Cuối job chạy **`npm run export-to-supabase`** (đẩy `public/data/batches` → Supabase), trừ khi đặt biến **`SKIP_EXPORT_TO_SUPABASE=1`** (Actions → Variables). Cùng **`EXPORT_TO_SUPABASE_SCOPE`** như workflow export riêng.
